...
def gen_frames():
    cap = cv2.VideoCapture(
        f"https://zm.apagaofogo.eco.br/zm/cgi-bin/nph-zms?mode=jpeg&monitor={zm_monitor_id}&scale=100&user=apagaofogo&pass=sng2Bu1Gyb4TCQPDrdC5HiXhIXW8hf"
    )
...
def salvar_ptz():
    ...
    try:
        ...
        cursor = conn.cursor()
        sql = """
        INSERT INTO camera_coordinates(`camera_id`, `pan`, `tilt`, `zoom`, `description`)
        VALUES (%s, %s, %s, %s, %s)
        """
        cursor.execute(
            sql,
            (data["cameras_id"], data["pan"], data["tilt"], data["zoom"], data["desc"]),
        )
        conn.commit()
