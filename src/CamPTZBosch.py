...
from CamBasePTZ import CamBasePTZ
import json
import time
        ...
        while True:
            ...
            if agora < deadline:
                ...
                if camera["ptz_ctrl_ativo"] == 1:
                    ...
                    # Novo algoritmo de ajuste de ptz
                    w,h = (self.config_cam["width"], self.config_cam["height"]) # image size [pixels]
                    wmax,hmax = (2560, 1440) # maximum image size [pixels]
                    W = 2688 # maximum camera image width [pixels]
                    H = 1520 # maximum camera image height [pixels]
                    polim = [0, 360] # camera pan limits [degrees]
                    tolim = [0, 95] # camera tilt limits [degrees]
                    dpmax = wmax / W
                    dtmax = hmax / H
                    plim = [-dpmax, dpmax] # camera space pan limits
                    tlim = [-dtmax, dtmax] # camera space tilt limits
                    L = 8.93 # camera sensor diagonal length [mm]
                    F = 6.6 # camera focal length at unit zoom factor [nm]
                    eta = 3 # target image fraction size reference
                    zomax = 30 # maximum camera zoom factor
                    p, t, z = self.move_camera_to_track_object(bbox_maior_score,ptz_atual,w,h,wmax,hmax,W,H,L,F,zomax,polim,tolim,plim,tlim,eta)
