...
class RTSPFrameGrabber(threading.Thread):
    ...
    def _record_failure(self, logger, reason: str):
        logger.warning(
            f"[Grabber] Failure: {reason}. Waiting {self._penalty_seconds}s before restart."
        )
        self._healthy_frames = 0
        self._corrupt_count = 0
        time.sleep(self._penalty_seconds)
        self._penalty_seconds = min(
            self._penalty_seconds * self.PENALTY_MULTIPLIER, self.PENALTY_MAX
        )
    ...
    def _record_corrupt_frame(self, logger) -> bool:
        ...
        self._healthy_frames = 0
        logger.warning(
            f"[Grabber] Corrupt frame {self._corrupt_count}/{self.CORRUPT_TOLERANCE}"
        )
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                ffmpeg_cmd = [
                    ...
                    self.rtsp_url,
                    "-vf",
                    f"fps=4,scale={self.width}:{self.true_height}",
                    "-c:v",
                    "mjpeg",
                    "-q:v",
                    "10",
