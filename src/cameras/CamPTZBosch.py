...
from cameras.CamBasePTZ import CamBasePTZ
import json
import time
...
#
class CameraPTZMBosch(CamBasePTZ):
    def conectar_ptz_bosch(self, camera):
        ptz = onvif_control.CameraControl(
            camera["ptz_ip"], camera["ptz_user"], camera["ptz_password"]
        )
        ptz.camera_start()
        return ptz
    ...
    def run(self):
        ...
        camera = self.update_ptz_camera(self.config_sys, self.config_cam)
        ...
        # Limitação de FPS
        frames_por_segundo = 1.0 / float(
            self.config_sys["detector"]["max_fps_processamento"]
        )
        ...
        if camera["ptz_ctrl_ativo"] == 1:
            ...
            self.logger.info(
                f"Início do período de busca na coordenada {tuple(coordenadas[idx_coord][1:4])}."
            )
            time.sleep(self.config_cam["ptz_delay_stream"])
        else:
            self.logger.warning(
                f"Controle ativo de PTZ desligado, iniciando o período de busca sem atuação no PTZ."
            )
        ...
        while True:
            ...
            if len(results) > 0:
                self.logger.warning(
                    f"Detectado {self.config_sys['detector']['CLASSES'][int(results[0]['class'])]} com Score {float(results[0]['score']):.3f} na BBox {list(map(int, results[0]['bbox']))}."
                )
                detection_count += 1
            ...
            if agora < deadline:
                ...
                if camera["ptz_ctrl_ativo"] == 1:
                    ...
                    #
                    self.logger.warning(
                        f"Ajustando o PTZ para confirmação em função do BBox {bbox_maior_score} de ({panA:.3f}, {tiltA:.3f}, {zoomA:.3f}) --> ({p:.3f}, {t:.3f}, {z:.3f})."
                    )
                    ptzcam.absolute_move(p, t, z)
                    time.sleep(self.config_cam["ptz_delay_stream"])
                else:
                    self.logger.warning(
                        f"Controle ativo de PTZ desligado, iniciando o período de confirmação sem atuação no PTZ."
                    )
            ...
            self.logger.info(
                f"Final da busca na coordenada {tuple(coordenadas[idx_coord][1:4])}: {len(analyzed_frames)} Frames Analisados com {detection_count} Detecções."
            )
            ...
            if emconfirmacao:
                ...
                processamentos_id = self.insere_dados_processamentos(
            ...
            if (detection_count / qt_analyzed_frames) >= camera["perc_frames_para_mp4"]:
                ...
                (
                    ...
                    percentual_area_bbox_media,
                ) = self.processar_bboxes(todos_results)
                metadados = {}
                ...
                metadados["perc_frames_para_mp4"] = self.config_cam[
                    "perc_frames_para_mp4"
                ]
                ...
                if camera["ptz_ctrl_ativo"] == 1:
                    ...
                    metadados["desccoordenadas_cameras"] = str(
                        coordenadas[idx_coord][4]
                    )
            ...
            if int(camera["ptz_ctrl_ativo"]) == 1:
                ...
                self.logger.info(
                    f"Início do período de busca na coordenada {coordenadas[idx_coord][1:4]}."
                )
            else:
                self.logger.warning(
                    f"Controle ativo de PTZ desligado, iniciando o período de busca sem atuação no PTZ."
