...
from cameras.RTSPFrameGrabber import RTSPFrameGrabber
import threading


class AoFBase(mp.Process, ABC):
    ...
    def get_frame(self):
        ...
        if not frame_available:
            if self.logger:
                self.logger.warning(
                    "[Consumer] No frame received in 5s — camera may be down or in penalty."
                )
        ...
        with self.lock:
            try:
                ...
                if frame_size == 0 or frame_size + 4 > self.shm_size:
                    self.logger.error(
                        f"[Consumer] Invalid frame size in shm: {frame_size}"
                    )
                    return None, None, None
    ...
    def save_all_analyzed_video(
        ...
        width = self.config_cam["image_width"]
        height = self.config_cam["image_height"]

        if len(analyzed_frames) == 0 or len(analyzed_frames_wbbox) == 0:
            self.logger.error(
                f"Variáveis de frames zeradas! Não foi possível gerar MP4."
            )
            return
    ...
    def draw_boxes(self, image, results):
        ...
        for box in results:
            ...
            cv2.rectangle(
                image,
                (x_text - pad, y_text - text_h - pad),
                (x_text + text_w + pad, y_text + pad),
                color,
                -1,
            )
    ...
    def _connect_rabbit(self):
        ...
        for i in range(retries):
            try:
                self.logger.warning(
                    f"Connecting to RabbitMQ (Attempt {i + 1}/{retries})..."
                )
    ...
    def send_frame_to_rabbit(self, frame, camera):
        ...
        if self.rabbit_channel is None:
            if self.logger:
                self.logger.error("Connection does not exists")
            return None

        if not isinstance(frame, (bytes, bytearray)):
            if self.logger:
                self.logger.error(f"Encoding Error: Expected bytes, got {type(frame)}.")
            return None
        ...
        try:
            ...
            if self.logger:
                self.logger.error(f"Rabbit Publish Error: {e}")
            try:
    ...
    def processar_bboxes(self, todos_results):
        img_width = self.config_cam["image_width"]
        img_height = self.config_cam["image_height"]
        todas_bboxes = []
