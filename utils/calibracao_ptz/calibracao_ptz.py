...
def recalcular_e_ajustar_ptz(
    bbox, ptz, frame_size,
    fov_v, fov_h,  # fov_v = 37.1, fov_h = 61.8 (Bosch Autodome 5100i IR, wide)
    proporcao_alvo, zoom_maximo, zoom_minimo
):
    ...
    print("Recalculando PTZ:",
          bbox, ptz, frame_size,
          f"fov_v_atual={fov_v_atual:.6f}", f"fov_h_atual={fov_h_atual:.6f}",
          "graus_por_px_h", graus_por_px_h, "graus_por_px_v", graus_por_px_v,
          proporcao_alvo, zoom_maximo, zoom_minimo,
          "=>", pan_novo, tilt_novo, zoom_novo)
    return [round(pan_novo, 3), round(tilt_novo, 3), round(zoom_novo, 3)]

def gen_frames():
    global bbox, show_bbox
    cap = cv2.VideoCapture(f"http://zm.apagaofogo.eco.br/zm/cgi-bin/nph-zms?mode=jpeg&monitor={zm_monitor_id}&scale=100&user=apagaofogo&pass=sng2Bu1Gyb4TCQPDrdC5HiXhIXW8hf")
